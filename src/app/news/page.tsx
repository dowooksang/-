...
async function getBoardPermissions(category: string) {
  ...
  // 기본값 fallback (소식/알림은 일반 뷰어는 LV1, 작성은 LV5 관리자 이상)
  return {
    readLevel: 1,
    writeLevel: 5,
  };
...
export default async function NewsPage({
  ...
  const { readLevel, writeLevel } = await getBoardPermissions('news');
  ...
  if (action === 'write') {
        ...
        <div className="bg-white min-h-screen w-full flex items-center justify-center py-32 text-black">
          <div className="text-center max-w-md px-6">
            ...
            <p className="text-gray-500 mb-6 leading-relaxed text-sm">
              해당 게시판에 글을 등록하려면 <strong>{LEVEL_NAMES[writeLevel]}</strong> 이상의 권한이 필요합니다.
            </p>
            <Link
              href="/news"
              className="bg-[#0A103D] text-white px-6 py-2.5 rounded-lg text-sm font-bold hover:bg-[#1a225c] transition-colors"
    ...
    return <NewsForm currentUser={currentUser as any} />;
  }
  ...
  if (action === 'edit' && id) {
    ...
    return <NewsForm initialData={post} isEdit={true} currentUser={currentUser as any} />;
  }
  ...
  if (action === 'detail' && id) {
    ...
    return <NewsDetail post={{ ...post, views: nextViews }} currentUser={currentUser} />;
  }

  // 5. 기본값: 글 목록 뷰
  const { data: posts } = await supabaseServer
    .from('posts')
    .select('id, title, author, author_id, created_at, views, is_notice, comments(id)')
    .eq('category', 'news')
  ...
  return <NewsList posts={(posts as any) || []} canWrite={canWrite} />;
}
